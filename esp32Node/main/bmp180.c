...
static void writeRegister8(uint8_t reg1, uint8_t reg2) {
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (BMP180_ADDRESS << 1) | I2C_MASTER_WRITE, 1 /* expect ack */);
	i2c_master_write_byte(cmd, reg1, 1);
	i2c_master_write_byte(cmd, reg2, 1);
	i2c_master_stop(cmd);
...
void readCalibration() {
	...
	AC4 = readRegister16(BMP085_REGISTER_CAL_AC4);
	AC5 = readRegister16(BMP085_REGISTER_CAL_AC5);
	AC6 = readRegister16(BMP085_REGISTER_CAL_AC6);
...
uint32_t readUncompensatedTemp() {
	writeRegister8(BMP085_REGISTER_CONTROL, BMP085_REGISTER_READTEMPCMD);
...
uint32_t readUncompensatedPressure(uint32_t mode) {
	writeRegister8(BMP085_REGISTER_CONTROL, BMP085_REGISTER_READPRESSURECMD);
	switch(mode) {
...
void x() {
	...
	ESP_LOGI(tag, "UT %d  X1 %d X2 %d B5 %d Temp = %.1foC = %.1foF", UT, X1, X2, B5, temp, centigrade_to_fahrenheit(temp));
...
void task_bmp180(void *ignore) {
	...
	readCalibration();
	ESP_LOGI(tag, "Params: AC1: %d, AC2: %d, AC3: %d, AC4:%d, AC5: %d, AC6: %d, B1: %d, B2: %d, MB: %d, MC: %d, MD: %d",
